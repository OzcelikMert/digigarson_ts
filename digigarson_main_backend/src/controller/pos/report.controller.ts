...
// @access  Public
// Pos Kasa Z Raporu
export async function getCaseReportHandler(req: Request, res: Response) {
    ...
    let data: any = checks.map((b: any) => {
        ...
        next.product.forEach((el: any) => {
            ...
            if (existProduct){
                existProduct.quantity += Number(el.quantity)
                existProduct.price +=  Number(el.price)
            } 
        ...
        next.payment.forEach((el: any) => {
            ...
            if (existPayment) existPayment.amount = Number(existPayment.amount) + Number(el.amount)
