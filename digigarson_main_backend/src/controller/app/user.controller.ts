...
import { sendSms } from "../../service/netgsm.service";
...
export async function updateUserPasswordHandler(req: Request, res: Response) {
    try {
        ...
        const data = req.body
        let user: any = await validatePassword({ email: email, password: password }) as UserDocument;
...
// @desc    update  user password
// @route   PUT /v1/app/users/testsms
// @access  Private
//Çalışan bilgilerini günceller branchine çalışan atar.
export async function testsms(req: Request, res: Response){
    let r = `${Math.floor(Math.random()*10)}${Math.floor(Math.random()*10)}${Math.floor(Math.random()*10)}${Math.floor(Math.random()*10)}${Math.floor(Math.random()*10)}${Math.floor(Math.random()*10)}`;
await sendSms({
    gsmno: "5302414759",
    message: `ççşiİxaqert₺€ test test Maydey Maydey`,
  });
await sendSms({
    gsmno: "5380423261",
    message: `Giriş Bilgisi: ${r}`,
  })
}