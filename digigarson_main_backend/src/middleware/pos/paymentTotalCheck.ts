...
const paymentTotalCheck = async (
    ...
    if(orders) {
        const checkOrders = orders.every(reqOrder => {
            reqOrder.quantity = Number(reqOrder.quantity);
            /// current orders
            ...
            // check
            if (
                tableOrders.quantity >= reqOrder.quantity + (tablePaidProduct ? tablePaidProduct.quantity : 0)
            ) {
                return true
            } else {
                return false
    ...
    let newPayments = payments.reduce((prev, next) => {
        return {
            ...prev,
            amount: Number((Number(prev.amount) + Number(next.amount))).toFixed(2)
        }
    }, { amount: 0.00, currency: "TL" });
