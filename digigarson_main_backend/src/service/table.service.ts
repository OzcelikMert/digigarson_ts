...
export function findTable(
  ...
  return Table.findOne(query, projection, options);
}
...
export function findTables(
  ...
  return Table.find(query, projection, options);
}
...
export async function findAndUpdate(
  ...
  return Table.findOneAndUpdate(query, update, {
    new: true,
    runValidators: true,
  });
}
...
export async function updateTotalPrice(tableId: string, totalPrice: number) {
  return await updateOne({ _id: tableId}, { $set: { "totalPrice": totalPrice } })
}
...
export async function getTableTotalPayments(tableId: string, target_currency: string = "TL") {
  const allPayments: any = await findTable({ _id: tableId }, { payments: 1 });
    ...
    total = { ...total, amount: Number(total.amount)}
  return total;
}
...
export function countTable(query: FilterQuery<TableDocument>) {
  return Table.countDocuments(query);
}
//masaya giriş yapar.
export async function tableInsertLog(tableId: string, logs: string[]) {
  return Table.updateOne({ _id: tableId }, { "$push": { "logs": { "$each": logs } } })
}
...
export async function getGroupByBusyTable(query: FilterQuery<TableDocument> = {}) {
  return Table.aggregate([{ '$match': query }, { '$group': { _id: "$busy", count: { '$sum': 1 } } }])
}
