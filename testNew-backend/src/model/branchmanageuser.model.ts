...
interface BranchManageUserDocument extends mongoose.Document {
    email: string;
    gsm_no: string;
    name: string;
    lastname: string;
...
BranchManageSchema.pre("save", async function (next: mongoose.HookNextFunction) {
    let branchmanage = this as BranchManageUserDocument;
...
BranchManageSchema.methods.comparePassword = async function (
    ...
    const branchManage = this as BranchManageUserDocument;
...
const BranchManageUserModel = mongoose.model<BranchManageUserDocument>("branchManageUser", BranchManageSchema);

export {
    BranchManageUserModel,
    BranchManageUserDocument
};
