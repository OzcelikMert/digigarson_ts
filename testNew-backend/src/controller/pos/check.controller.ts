...
// @access  Private
//pos olarak görev yaptığı branch e ödemelerini gösterir.
getOld: async function (req: Request, res: Response) {
    const branchId = get(req, "user.branchId");
...
updateOld: async function (req: Request, res: Response) {
    const branchId = get(req, "user.branchId");
    const checkId = get(req.params, "checkId");
    ...
    let changedPayments = [...check.payments.map((currentPayment: any) => {
        ...
        if (!newPayment) return { ...currentPayment, amount: currentPayment.amount * -1 }
        return { ...currentPayment, amount: newPayment.amount - currentPayment.amount }
