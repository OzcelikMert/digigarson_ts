...
export default function Oldcreditcheck({ props }: { props: any }) {
    const {
        handleModal,
        checkID, table,
        setCurrentModal,
    ...
    const printData = {
        ...Fetchs.Check.OLD.data, orders: oldCheck?.products?.map((product: any) => ({
            name: product.productName,
            quantity: product.quantity,
            price: product.price
        })),
        ...Fetchs.Check.OLD.data, total: oldCheck?.payments?.map((payments: any) => (
            {
        ...
        <>
            <div className='oldCheckContainer'>
                <table style={{ width: "90%", border: "1px solid" }} >
                    ...
                    <tr style={{ width: "50%" }} >
                        <th>
                            {oldCheck?.is_it_paid ? t("paid") : ("not-paid")}
                        </th>
                        <th>
            ...
            <div className='changePriceButtons'>
                <button style={{ background: "blue" }} onClick={() => setCurrentModal(MODAL.GET_CREDIT_LIST)}> {t("back-to-list")} </button>
                <button style={{ background: "green" }} onClick={() => Printer.printAll(printData)}> {t("print")} </button>
            </div>
        </>
    )
}